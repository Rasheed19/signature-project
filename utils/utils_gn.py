...
class FeatureTransformation:
    ...
    def fit_transform(self, data, targets, with_eol, sig_level=None, multi_cycle=True):
        ...
        if self.feature_selection:
            ...
            self.selected_feature_names = df_features_only.columns[
                self.rrct.selected_features_indices_
            ]
            ...
            return (
                self.sc.transform(self.rrct.select(df_features_only.values)),
                df[targets].values,
            )
...
def load_and_save_dict_data(num_cycles=None, option=1):
    ...
    if option == 1:
        ...
        with open(
            os.path.join(f"{ROOT_DIR}/data", "data_" + filename_suffix), "wb"
        ) as fp:
        ...
        for i, batch in zip(("1", "2", "3", "4", "5", "6", "7", "8"), batches):
            with open(
                os.path.join(f"{ROOT_DIR}/data", "batch" + i + "_" + filename_suffix),
                "wb",
            ) as fp:
