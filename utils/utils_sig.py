...
def ccv_signature_features(data_dict, step_size=1, n=50, sig_level=2, multi_cycle=True, return_ccv=False, return_sig=False):
    """
    ...
    def evolution_fn(x):
        return [x.min(), x.max(), x.mean(), x.var(), scipy.stats.skew(x),
    ...
    for cell in data_dict.keys():
        ...
        for cycle in data_dict[cell]['cycle_dict'].keys():
            ...
            t_values = utils_gn.get_charge_discharge_values(data_dict, 't', cell, cycle, 'di')
    ...
    sig_multi_df = pd.DataFrame(data=np.array(sig_multi_features),
                                columns=[ft + item for ft in ['S1-', 'S2-', 'S11-', 'S12-', 'S21-', 'S22-'] for item in
                                         utils_gn.strings_multi_cycle_features(n)],
    ...
    return sig_multi_df.join(
        pd.DataFrame(data=np.array(sig_evolution_features),
                     columns=[stat + comp for comp in ['S1', 'S2', 'S11', 'S12', 'S21', 'S22'] for stat in
...
def plot_ccv_features(data_dict, fname, ylabel=None, ylim=None, sample_cells=None, option=1):
   """
   ...
   if option == 1:
        ...
        for i, cell in enumerate(sample_cells):
            ...
            ax.annotate('Decreasing', xy=(10, 2.6), xytext=(8, 3.3),
                               arrowprops=dict(facecolor='black', linewidth=0.1), size=16)
