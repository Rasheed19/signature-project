...
def create_knee_elbow_data(data_dict, with_eol=True):
    ...
    if with_eol:
        ...
        for cell in data_dict.keys():
            ...
            knee_elbow_data.loc[cell, ["k-o", "k-p", "Qatk-o", "Qatk-p"]] = (
                utils_models.knee_elbow_detection(
                    x_data=np.arange(len(qd)) + 1, y_data=qd, type="knee"
                )
            )
            knee_elbow_data.loc[cell, ["e-o", "e-p", "IRate-o", "IRate-p"]] = (
                utils_models.knee_elbow_detection(
                    x_data=np.arange(len(ir)) + 1, y_data=ir, type="elbow"
                )
            )
            cleaned_ir = utils_models.knee_elbow_detection(
        ...
        for cell in data_dict.keys():
            qd = data_dict[cell]["summary"]["QDischarge"]
            ir = data_dict[cell]["summary"]["IR"]
            ttk_o, ttk_p, _, q_at_k_o, q_at_k_p = utils_models.knee_elbow_detection(
                x_data=np.arange(len(qd)) + 1, y_data=qd, type="knee"
            )
            knee_elbow_data.loc[cell, ["k-o", "k-p", "Qatk-o", "Qatk-p"]] = (
                ttk_o,
