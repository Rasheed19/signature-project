...
def model_feature_selection_robustness(train_raw, test_raw, y_test_df, target_list, params, step_size_dict, times_needed, k):
    ...
    for h in times_needed:
        ...
        # Transforms raw data to training data
        tr = utils_gn.FeatureTransformation(n=100, feature_selection=True, k=k, step_size=h)
        X_train, y_train = tr.fit_transform(data=train_raw, targets=target_list, with_eol=True, sig_level=2, multi_cycle=False)
        X_test, y_test = tr.transform(test_raw, sig_level=2, multi_cycle=False), y_test_df[target_list].values
       ...
       # Fit the model
        ...
        # Update tracker
        track_metrics.loc[step_size_dict[h], ['Selected features', 'MAE_train', 'MAPE_train', 'RMSE_train', 'MAE_test', 'MAPE_test', 'RMSE_test']] =      \
                                                                          tr.selected_feature_names[0], train_scores['MAE'], train_scores['MAPE'], train_scores['RMSE'], \
                                                                          test_scores['MAE'], test_scores['MAPE'],test_scores['RMSE']
...
def test_of_robustness(model, model_tr, time_steps, X_test_data, y_test_data, targets, step_size_dict):
    ...
    for t in time_steps:
        ...
        X_test = utils_sig.ccv_signature_features(data_dict=X_test_data, step_size=t, n=100, sig_level=2, multi_cycle=False).values
...
class ModelPipeline:
    ...
    def __init__(self, params, transform_target):
        ...
        self.transform_target = transform_target  # bool; whether to log-transform target or not
